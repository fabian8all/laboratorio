...
$(document).ready(function () {
  $("#bstableClientes").bootstrapTable({
    queryParams: function queryParams(p) {
      return {
        search: p.search,
        limit: p.limit,
        offset: p.offset,
  ...
  $.confirm({
    ...
    confirm: function confirm() {
      $.post("routes/routeClientes.php", {
        ...
        if (data == "true") customAlert("Exito!", "El cliente ha sido eliminado");else customAlert("Error!", "Error al intentar eliminar al cliente");
        load_clientes_data();
      }).fail(function (error) {
        customAlert("Error!", ajaxError);
      });
